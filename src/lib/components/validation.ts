...
function validateComponentBlockProps(schema: ComponentSchema, value: unknown, path: ReadonlyPropPath): any {
  if (schema.kind === 'form') {
  ...
  if (schema.kind === 'conditional') {
    ...
    const obj: any = {}
    const discriminantVal = validateComponentBlockProps(schema.discriminant, discriminant, path.concat('discriminant'))
    if (discriminantVal !== undefined) {
      obj.discriminant = discriminantVal
    }
    const conditionalFieldValue = validateComponentBlockProps(schema.values[discriminant], val, path.concat('value'))
  ...
  if (schema.kind === 'object') {
    ...
    for (const key of Object.keys(schema.fields)) {
      const propVal = validateComponentBlockProps(schema.fields[key], (value as any)[key], path.concat(key))
      // for some reason mongo or mongoose or something is saving undefined as null
      // so we're doing this so that we avoid setting undefined on objects
  ...
  if (schema.kind === 'array') {
    ...
    return value.map((innerVal, i) => {
      return validateComponentBlockProps(schema.element, innerVal, path.concat(i))
    })
  }
  assertNever(schema)
}
...
export function getValidatedNodeWithNormalizedComponentFormProps(
  ...
  if (node.type === 'component-block') {
    if (componentBlocks.hasOwnProperty(node.component)) {
      ...
      node = {
        ...node,
        props: validateComponentBlockProps({ kind: 'object', fields: componentBlock.schema }, node.props, []),
      }
    }
  }
  ...
  if (node.type === 'relationship') {
    node = {
      type: 'relationship',
      data: node.data?.id !== undefined ? { id: node.data.id, data: undefined, label: undefined } : null,
  ...
  return {
    ...node,
    children: node.children.map((x) => getValidatedNodeWithNormalizedComponentFormProps(x, componentBlocks)),
  }
}
...
export function validateAndNormalizeDocument(
  ...
  const children = value.map((x) => getValidatedNodeWithNormalizedComponentFormProps(x, componentBlocks))
