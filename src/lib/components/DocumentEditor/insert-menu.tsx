...
function getOptions(toolbarState: ToolbarState, componentBlocks: Record<string, ComponentBlock>): Option[] {
...
export function InsertMenu({ children, text }: { children: ReactNode; text: Text }) {
  ...
  const { dialog, trigger } = useControlledPopover({ isOpen: true, onClose: noop }, { placement: 'bottom-start' })
    ...
    if (editor.selection && text === '/') {
      ...
      if (
        ...
        (Point.equals(startOfBlock, before) ||
          (before.offset !== 0 && /\s/.test((Node.get(editor, before.path) as Text).text[before.offset - 1])))
      ) {
        Transforms.setNodes(
          editor,
          { insertMenu: true },
