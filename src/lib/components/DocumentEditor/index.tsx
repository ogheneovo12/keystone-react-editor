  ...
  return (
    <pre>
      {JSON.stringify(
        {
          selection:
            editor.selection && Range.isCollapsed(editor.selection) ? editor.selection.anchor : editor.selection,
...
type TypesWhichHaveNoExtraRequiredProps = {
  [Type in Block['type']]: { type: Type; children: Descendant[] } extends Block & { type: Type } ? Type : never
}[Block['type']]

const blockquoteChildren = ['paragraph', 'code', 'heading', 'ordered-list', 'unordered-list', 'divider'] as const
...
function inlineContainer(args: { invalidPositionHandleMode: 'unwrap' | 'move' }): InlineContainerSchema {
  return {
    kind: 'inlines',
    invalidPositionHandleMode: args.invalidPositionHandleMode,
  }
...
export const editorSchema = satisfies<Record<Block['type'] | 'editor', BlockContainerSchema | InlineContainerSchema>>()(
  {
...
const blockTypes: Set<string | undefined> = new Set(Object.keys(editorSchema).filter((x) => x !== 'editor'))
      ...
      for (const [index, childNode] of node.children.entries()) {
        ...
        if (info.kind === 'inlines') {
          ...
          if (
            ...
            Transforms.wrapNodes(editor, { type: info.blockToWrapInlinesIn, children: [] }, { at: childPath })
            return
          }
          if (
...
function handleNodeInInvalidPosition(editor: Editor, [node, path]: NodeEntry<Block>, parentPath: Path) {
