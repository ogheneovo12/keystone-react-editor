...
export const isListType = (type: string | undefined) => type === 'ordered-list' || type === 'unordered-list'

export const isListNode = (node: Node): node is Element & { type: 'ordered-list' | 'unordered-list' } =>
  ...
  if (previousListItemNode.children.length !== 1) {
    ...
    Transforms.moveNodes(editor, {
      ...
      to: [
        ...
        (previousListItemNode.children[previousListItemNode.children.length - 1] as any).children.length,
      ],
    })
    return true
  }
  const type = Editor.parent(editor, Path.parent(block[1]))[0].type as 'ordered-list' | 'unordered-list'
  Editor.withoutNormalizing(editor, () => {
