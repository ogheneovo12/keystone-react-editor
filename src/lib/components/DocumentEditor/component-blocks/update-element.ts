    ...
    for (const childProp of childrenLeftToAdd) {
      Transforms.insertNodes(
        editor,
        {
          ...
          children: [
            childProp.options.kind === 'block' ? { type: 'paragraph', children: [{ text: '' }] } : { text: '' },
          ],
        },
        { at: [...basePath, newIdx] },
      )
  ...
  switch (schema.kind) {
    ...
    case 'conditional':
      ...
      return findChildPropPathsWithPrevious(
        value.value,
        hasChangedDiscriminant ? prevValue.value : getInitialPropsValue(schema.values[value.discriminant]),
