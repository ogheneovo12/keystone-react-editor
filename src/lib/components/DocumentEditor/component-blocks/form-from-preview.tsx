    ...
    <Stack gap="medium">
      ...
      <OrderableList {...props}>
        {props.elements.map((val) => {
          return <OrderableItemInForm elementKey={val.key} label={props.schema.itemLabel?.(val) ?? 'Item'} {...val} />
        })}
      </OrderableList>
      <Button
    ...
    <OrderableItem elementKey={props.elementKey}>
      <Stack gap="medium">
        <div css={{ display: 'flex', gap: 4 }}>
          ...
          >
            <span css={{ fontSize: 16, fontWeight: 'bold', textAlign: 'start' }}>{props.label}</span>
          </Button>
          <RemoveButton />
        </div>
