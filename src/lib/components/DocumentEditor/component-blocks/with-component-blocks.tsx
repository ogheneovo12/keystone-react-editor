...
function getAncestorComponentBlock(editor: Editor) {
  if (editor.selection) {
    ...
    if (
      ancestorEntry &&
      (ancestorEntry[0].type === 'component-block-prop' || ancestorEntry[0].type === 'component-inline-prop')
    ) {
      return {
        isInside: true,
        componentBlock: Editor.parent(editor, ancestorEntry[1]) as NodeEntry<Element & { type: 'component-block' }>,
        prop: ancestorEntry as NodeEntry<Element & { type: 'component-inline-prop' | 'component-block-prop' }>,
      } as const
    }
  }
...
function normalizeNodeWithinComponentProp(
  ...
  if (Element.isElement(node)) {
    let childrenHasChanged = node.children
      .map((node, i) => normalizeNodeWithinComponentProp([node, [...path, i]], editor, fieldOptions))
      // .map then .some because we don't want to exit early
      .some((x) => x)
    if (fieldOptions.kind === 'block') {
      didNormalization =
        normalizeElementBasedOnDocumentFeatures([node, path], editor, fieldOptions.documentFeatures) ||
        childrenHasChanged
    } else {
...
function isEmptyChildFieldNode(
  ...
  return (
    ...
    ((element.type === 'component-inline-prop' && firstChild.type === undefined && firstChild.text === '') ||
    ...
    if (node.type === 'component-block') {
      ...
      if (componentBlock) {
        ...
        for (const [propPath, arrayField] of findArrayFieldsWithSingleChildField(rootSchema, node.props)) {
          ...
          const nodesWithin: [number, Element & { type: 'component-block-prop' | 'component-inline-prop' }][] = []
          ...
          if (!areArraysEqual(arrVal, newVal)) {
            ...
            Transforms.setNodes(editor, { props: transformedProps as Record<string, unknown> }, { at: path })
