...
export function getInitialValue(type: string, componentBlock: ComponentBlock) {
  ...
  return {
    ...
    children: findChildPropPaths(props, componentBlock.schema).map((x) => ({
      ...
      propPath: x.path,
      children: [x.options.kind === 'block' ? { type: 'paragraph' as const, children: [{ text: '' }] } : { text: '' }],
    })),
  }
}
...
export function getInitialPropsValueFromInitializer(schema: ComponentSchema, initializer: any): any {
  switch (schema.kind) {
    case 'form':
      ...
      return {
        ...
        value:
          updater.discriminant === currentValue.discriminant
            ? updateValue(schema.values[updater.discriminant.toString()], currentValue.value, updater.value)
