...
export function ComponentBlockRender({
  ...
  const getPreviewProps = useMemo(() => {
    return createGetPreviewProps({ kind: 'object', fields: componentBlock.schema }, onChange, (path) => (
      <ChildFieldEditable path={path} />
    ))
  ...
  return (
    <ChildrenByPathContext.Provider value={childrenByPath}>
      ...
      <span css={{ caretColor: 'transparent', '& ::selection': { backgroundColor: 'transparent' } }}>{maybeChild}</span>
