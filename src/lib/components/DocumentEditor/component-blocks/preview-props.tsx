...
export function createGetPreviewProps<Schema extends ComponentSchema, ChildFieldElement>(
  rootSchema: Schema,
  rootOnChange: (cb: (val: ValueForComponentSchema<Schema>) => ValueForComponentSchema<Schema>) => void,
  ...
  const memoizedInfoForSchema = castToMemoizedInfoForSchema({
      ...
      return {
        ...
        inner: new Map<
          string,
          {
            ...
            elementWithKey: { key: string } & GenericPreviewProps<ComponentSchema, ChildFieldElement>
    ...
    relationship(schema, onChange) {
      return (newVal: HydratedRelationshipData | readonly HydratedRelationshipData[] | null) => onChange(() => newVal)
    },
  })
  ...
  const previewPropsFactories: {
    [Kind in ComponentSchema['kind']]: (
      ...
      getInnerProp: <Field extends ComponentSchema>(
        ...
        onChange: (cb: (prevVal: ValueForComponentSchema<Field>) => ValueForComponentSchema<Field>) => void,
        key: string,
    ...
    object(schema, value, memoized, path, getInnerProp) {
      ...
      const previewProps: GenericPreviewProps<ObjectField<Record<string, ComponentSchema>>, ChildFieldElement> = {
        fields,
  ...
  function getPreviewPropsForProp<Schema extends ComponentSchema>(
    ...
    return previewPropsFactories[schema.kind](schema as any, value, memoedThing as any, path, getInnerProp) as any
  }

  function getInitialMemoState<Schema extends ComponentSchema>(
    schema: Schema,
    value: ValueForComponentSchema<Schema>,
    onChange: (cb: (val: ValueForComponentSchema<Schema>) => ValueForComponentSchema<Schema>) => void,
    path: readonly string[],
  ): MemoState<Schema> {
  ...
  return (value: ValueForComponentSchema<Schema>): GenericPreviewProps<Schema, ChildFieldElement> => {
