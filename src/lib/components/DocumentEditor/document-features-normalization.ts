...
export function normalizeElementBasedOnDocumentFeatures(
  [node, path]: NodeEntry<Element>,
  editor: Editor,
  { formatting, dividers, layouts, links, relationships: relationshipsEnabled }: DocumentFeaturesForNormalization,
): boolean {
  if (
    (node.type === 'heading' && (!formatting.headingLevels.length || !formatting.headingLevels.includes(node.level))) ||
    ...
    (node.type === 'layout' && (layouts.length === 0 || !layouts.some((layout) => areArraysEqual(layout, node.layout))))
  ) {
    Transforms.unwrapNodes(editor, { at: path })
    return true
  }
  ...
  return normalizeInlineBasedOnLinksAndRelationships([node, path], editor, links, relationshipsEnabled)
}

export function withDocumentFeaturesNormalization(documentFeatures: DocumentFeatures, editor: Editor): Editor {
  ...
  editor.normalizeNode = ([node, path]) => {
    if (Text.isText(node)) {
      ...
      normalizeElementBasedOnDocumentFeatures([node, path], editor, documentFeaturesForNormalization)
    }
