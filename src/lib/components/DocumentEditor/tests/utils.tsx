...
import { defaultDocumentFeatures } from '../defaultComponentFeatures'
...
console.error = (...stuff: any[]) => {
  ...
  console.log('console.error was called, either add an exception for this kind of call or fix the problem')
...
expect.extend({
  toEqualEditor(received: Editor, expected: Editor) {
    ...
    validateAndNormalizeDocument(received.children, receivedConfig.documentFeatures, receivedConfig.componentBlocks)

    const expectedConfig = (expected as any).__config
    validateAndNormalizeDocument(expected.children, expectedConfig.documentFeatures, expectedConfig.componentBlocks)
...
function EditorComp({
  ...
  return (
    <Slate editor={editor} initialValue={val} onChange={setVal}>
      <ToolbarStateProvider componentBlocks={componentBlocks} editorDocumentFeatures={documentFeatures}>
...
export const makeEditor = (
  ...
  let editor = createDocumentEditor(documentFeatures, componentBlocks) as Editor & {
  ...
  if (skipRenderingDOM !== true) {
    ...
    const { container } = render(
      <EditorComp editor={editor} componentBlocks={componentBlocks} documentFeatures={documentFeatures} />,
    )
...
function nodeToReactElement(editor: Editor, node: Node, selection: Range | null, path: Path): ReactElement {
  if (Text.isText(node)) {
  ...
  let children = node.children.map((x, i) => nodeToReactElement(editor, x, selection, path.concat(i)))
  if (Editor.isEditor(node)) {
    const config = (editor as any).__config
    validateAndNormalizeDocument(node.children, config.documentFeatures, config.componentBlocks)
...
const editorSerializer: Parameters<typeof expect.addSnapshotSerializer>[0] = {
  ...
  serialize(val, config, indentation, depth, refs, printer) {
    return printer(nodeToReactElement(val, val, val.selection, []), config, indentation, depth, refs)
  },
}
